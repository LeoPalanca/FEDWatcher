...
\[
  Y^*_t = X_t \beta + \epsilon_t
  \Pr(Y_{t} = j \mid \mathcal{X}_t) = \frac{\exp{\mathcal{X}_t}\beta_j}{1+\sum_{k=1}^J\exp{\mathcal{X}_t}}),\qquad c_0 = -\infty,\ c_6 = +\infty,
\]
...
\[
  \text{S}_t = \alpha_t \cdot \text{tone\_score}_t + (1 - \alpha_t) \cdot \text{S}_{t-1}, \alpha_t = 1 - e^{-\lambda \Delta t}, \quad \text{where } \lambda = \frac{\ln(2)}{h}.
\]
