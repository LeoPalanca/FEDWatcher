...
This version is for the third sentiment pipeline.

Reads documents from:
    documents(id, central_bank, doc_type, release_date, url, raw_text, processed3)

Uses ONLY:
    documents.processed3
...
def insert_sentiment3(conn: sqlite3.Connection, result: ToneResult) -> None:
    row = result.to_db_row()

    conn.execute(
        """
        INSERT INTO sentiment3 (
            document_id,
            overall_tone,
            tone_score,
            inflation_assessment,
...
def mark_document_processed3(conn: sqlite3.Connection, document_id: int) -> None:
    conn.execute(
        """
        UPDATE documents
        SET processed3 = 1
...
def mark_document_failed3(conn: sqlite3.Connection, document_id: int) -> None:
    """
    Keeps failed documents unprocessed for this third pipeline.
    Does not touch documents.processed or documents.processed2.
    """
    conn.execute(
        """
        UPDATE documents
...
def process_unprocessed3_documents(
    db_path: str,
    limit: int,
    dry_run: bool = False,
) -> int:
    ...
    agent = AnalystAgent()
    documents = fetch_unprocessed3_documents(conn, limit=limit)

    if not documents:
        print("No documents with processed3 = 0 found.")
        conn.close()
        return 0

    print(f"Found {len(documents)} document(s) with processed3 = 0.")
    ...
    try:
        for document in documents:
            ...
            try:
                ...
                if dry_run:
                    print(
                        "Dry run enabled: not writing to sentiment3 "
                        "and not updating processed3."
                    )
                    continue

                insert_sentiment3(conn, result)
                mark_document_processed3(conn, doc_id)
                conn.commit()

                processed_count += 1
                print(
                    f"Saved to sentiment3 and marked document {doc_id} "
                    "as processed3 = 1."
                )

            except Exception as exc:
                conn.rollback()
                mark_document_failed3(conn, doc_id)
                conn.commit()
                print(
    ...
    print("-" * 80)
    print(f"Done. Processed {processed_count} document(s) into sentiment3.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze documents where processed3 = 0 and write output to sentiment3."
        )
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run analysis but do not write to sentiment3 or update processed3.",
    )
...
def main() -> None:
    ...
    process_unprocessed3_documents(
        db_path=args.db,
        limit=args.limit,
